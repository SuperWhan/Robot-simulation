 ...
 ******************************************************************************/
/**
 * @brief Class representing the battery for a Player.
 */
class PlayerBattery {
 public:
  ...
  /**
   * @brief All PLayers consume SOME power, even when just sitting there not
  ...
  /**
   * @brief The amount of energy consumed by the Player due to a collision event
   ...
   *
   * @param old_pos The player's previous position.
   * @param new_pos The player's current position.
   * @param dt Time taken by the player to move from old_pos to new_pos.
   *
   * @return The updated battery level.
   */
  ...
  double Dist(Position old_pos, Position new_pos);

 ...
 *
 * This is RobotSim v3.0. It is a mini video game allowing the player to
 ...
 * moves. When the battery level goes to 0, the player lose the game. 
 * If 5 robots are all transfer into superbot, then you lose the game too.
 * The player can control the palyer_ through the 4 arrow keys, but be
 * careful--bumping into obstacles will both deplete some battery and slow
 * down the robot, or if you are hitting on the superbot, the player will be
 * frozed for like few seconds(during this time, you are not able to do anything).
 *
 ...
 *
 *
 * \section caution_fixed Caution different with v2.0 
 * Caution: Most bugs should be in the BugReport.
 * The difference between the Robotsim3.0 with 2.0 version, I fixed the SUperbot part
 * Since spend the whole night in the lind, still can't get the Sensors's functionalities.
 * So the biggest issue is still on the Sensors. None of them are working (sorry, I tried 
 * hard), good news is the game way changed since the robot could change into superbot.
 * 
 */

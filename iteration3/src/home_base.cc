...
HomeBase::HomeBase(const struct home_base_params *const params) :
  ...
  // int->double#include "src/player_motion_handler.h"  //changed
...
void HomeBase::Reset() {
  ...
  motion_handler_.set_heading_angle(270.0);  // int->double
  motion_handler_.set_speed(5);
  motion_handler_.max_speed(10);
  sensor_touch_.Reset();
} /* Reset */
...
void HomeBase::TimestepUpdate(uint dt) {
  ...
  if (random_num(0, 50 + 1) == 0) {
    motion_handler_.set_heading_angle(random_num(0.0,
                                      motion_handler_.max_angle()));
  }
} /* TimestepUpdate */
